    ...
    def city(self):
        ...
        try:
            ...
            #------getting the URL request----
                ...
                if(i==1):
                    response = requests.get(f'https://collegedunia.com/study-abroad/{self.course_name}-universities', headers={'User-Agent': random.choice(user_agents_list)})
                else:
                    response = requests.get(f'https://collegedunia.com/study-abroad/{self.course_name}-universities/page-'+str(i), headers={'User-Agent': random.choice(user_agents_list)})
                ...
                for university in universities:        
                    ...
                    if not living == None:
                        self.living_fees.append(living.text.split()[3])
                    else:
                        self.living_fees.append("-")
                

                    rate = university.find('span', class_="jsx-3255989801 rating f-13")
                    if not rate == None:
                        self.ratings.append(rate.text.split("/")[0])
                    else:
                        self.ratings.append("-")
